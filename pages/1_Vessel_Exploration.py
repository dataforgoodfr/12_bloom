...
from bloom.infra.database.sql_model import Vessel
...
options = [None,246014000, 224068000, 277510000, 205775000, 224048290, 228110000, 263576000, 224148000, 238561110, 237034000]
other_mmsis = [int(x) for x in list(trawlers["mmsi"].unique()) if not pd.isnull(x) and int(x) not in options[1:]]
...
with st.sidebar.form("Vessel selection"):
    ...
    # vessel_name = st.selectbox("Select from vessel name", options_names)
    submitted = st.form_submit_button("Load",on_click=select_vessel,args=(mmsi,))
# mmsi = st.number_input("Vessel mmsi",step = 1)
...
if st.session_state.vessel_mmsi is None:
    ...
    #     st.session_state.mpas = v.mpas
    # else:
    #     if hasattr(v,"_mpas"):
    #         v.mpas = st.session_state.mpas
    ...
    with col2:
        min_value,max_value = st.slider("Chunk id",min_value = 0,max_value = len(v.positions),value=st.session_state.range,step = 5)
    ...
    col3.metric("Duration",str(vi.positions["timestamp"].max() - vi.positions["timestamp"].min()).split(".")[0][:-3])
    col4.metric("Average speed",round(vi.positions["speed"].mean(),2))

    if show_mpas:
        vi.get_closest_marine_protected_areas()

    folium_map = vi.visualize_trajectory(with_animation = False,marker_by_fishing = show_markers_fishing,show_mpas = show_mpas,show_iucn = True)
    ...
    # reload_mpas = st.button("Reload MPAs")
    # if reload_mpas:
    #     vi.get_closest_marine_protected_areas()
    #     st.session_state.mpas = vi.mpas
