...
def add_excursion(
    ...
    new_excursion = excursion_repository.create_excursion(session, new_excursion) # ERREUR ICI
...
def get_time_of_departure(df: pd.DataFrame, a: int) -> datetime:
    if df["type"].iloc[a] == "DEFAULT_AIS":
        return df["timestamp_end"].iloc[a] - timedelta(
            0, 3600 * df["distance"].iloc[a] / average_exit_speed
        )
    else:
...
def run():
    ...
    excursion_repository = use_cases.excursion_repository()
    metrics_repository = use_cases.metrics_repository()
    ...
    with db.session() as session:
        ...
        last_segments = segment_repository.get_last_vessel_id_segments(session)
        last_segments["longitude"] = None
        last_segments["latitude"] = None
        last_segments = last_segments.apply(to_coords, axis=1)

        result = pd.DataFrame()

        nb_new_segments = 0
        nb_updated_excursions = 0
        nb_new_rels = 0
        nb_new_metrics = 0
        nb_last_segments_updated = 0

        logger.info("Création des nouveaux segments, mise à jour et création des excursions, création des métriques.")

        for vessel_id in tqdm(batch["vessel_id"].unique()):
            ...
            for col in df_start.columns:
                df_start.rename(
                    columns={col: col.replace("end", "start")}, inplace=True
                )

            vessel_last_segment = last_segments.loc[
                last_segments["vessel_id"] == vessel_id,
                [
                    "mmsi",
                    "timestamp_end",
                    "heading_at_end",
            ...
            vessel_last_segment["start_latitude"] = vessel_last_segment[
                "end_position"
            ].apply(lambda x: x.y)
            vessel_last_segment["start_longitude"] = vessel_last_segment[
                "end_position"
            ].apply(lambda x: x.x)

            vessel_last_segment.drop("end_position", inplace=True, axis=1)
            vessel_last_segment.rename(
                columns={
            ...
            if vessel_last_segment.shape[0] > 0:
                ...
                if len(df_start) >= 1 and len(df_end) > 1:
                ...
                if vessel_last_segment["arrival_port_id"].iloc[0] >= 0:
                    ongoing_excursion = False
                else:
                    ongoing_excursion = True
                    excursion_id = int(vessel_last_segment["excursion_id"].iloc[0])
                ...
                for a in df.index:
                    if df["port"].iloc[a] is not None and df["port"].iloc[a] >= 0:
                        if ongoing_excursion:
                            ...
                            )  # put the close excursion function here
                            df["excursion_id"].iloc[a] = excursion_id
                            ongoing_excursion = False
                            nb_closed_excursion += 1
                    elif ongoing_excursion:
                        df["excursion_id"].iloc[a] = excursion_id
                    else:
                        if is_new_vessel:
                            excursion_id = add_excursion(
                                session,
                                int(vessel_id),
                                df["timestamp_end"].iloc[a],
                                Point(
                            ...
                            excursion_id = add_excursion(
                                session, int(vessel_id), get_time_of_departure(df, a)
                            )  # put the create new excursion function here
                            nb_created_excursion += 1
                        ongoing_excursion = True
                        df["excursion_id"].iloc[a] = excursion_id

                if (df[df["excursion_id"] >= 0].shape[0] > 0):
                    result = df[df["excursion_id"] >= 0].reset_index()
                    ...
                    new_segments = segment_repository.batch_create_segment(session, new_segments)

                    segments_ids = set(seg.id for seg in new_segments)

                    result = segment_repository.find_segments_in_zones_by_ids(
                        session, segments_ids
                    )
                    new_rels = []
                    new_metrics = []
                    excursions = {}
                    segments = []
                    i = 0
                    vessel_attributes = vessel_repository.get_vessel_by_id(
                        session, int(vessel_id)
                    )
                    ...
                    for segment, zones in result.items():
                        ...
                        for zone in zones:
                            ...
                            if zone.category == "amp":
                                ...
                                types = "in_territorial_water"

                            if types != "AT_SEA":

                                new_metric = Metrics(  # 1
                                    ...
                                    duration_fishing=(
                                        segment.segment_duration if segment.type == "FISHING" else None
                                    ),
                                    zone_id=zone.id,
                        ...
                        excursion = excursions.get(
                            segment.excursion_id,
                        ...
                        if segment.in_zone_with_no_fishing_rights:
                            ...
                            if segment.type == "FISHING":
                                excursion.total_time_fishing_in_zones_with_no_fishing_rights += (
                                    segment.segment_duration
                                )
                        if segment.in_territorial_waters:
                            ...
                            if segment.type == "FISHING":
                                excursion.total_time_fishing_in_territorial_waters += (
                                    segment.segment_duration
                                )
                        ...
                        if segment.updated_at and segment.updated_at > point_in_time:
                            point_in_time = segment.updated_at
                        elif segment.created_at > point_in_time:
                            point_in_time = segment.created_at

                    excursion_repository.batch_update_excursion(session, excursions.values())
                    nb_updated_excursions += len(excursions.values())

                    segment_repository.batch_update_segment(session, segments)
                    nb_new_segments += len(segments)

                    RelSegmentZoneRepository.batch_create_rel_segment_zone(session, new_rels)
                    nb_new_rels += len(new_rels)

                    metrics_repository.batch_create_metrics(session, new_metrics)
                    nb_new_metrics += len(new_metrics)
                    if len(excursions) > 0:
                        nb_last = segment_repository.update_last_segments(session, [int(vessel_id)])
                        nb_last_segments_updated += nb_last

        logger.info(f"{nb_updated_excursions} excursions mises à jour")
        logger.info(f"{nb_new_segments} segments mis à jour")
        logger.info(f"{nb_new_rels} associations(s) créées")
        logger.info(f"{nb_new_metrics} metrics(s) créés")
        logger.info(f"{nb_last_segments_updated} derniers segments mis à jour")

        session.commit()

        now = datetime.now(timezone.utc)
        TaskExecutionRepository.set_point_in_time(
            session, "create_update_excursions_segments", now
        )
        session.commit()

        if point_in_time:
            TaskExecutionRepository.set_duration(
                session,
                "create_update_excursions_segments",
                now,
        ...
        if now != None:
            TaskExecutionRepository.set_position_count(
                session, "create_update_excursions_segments", now, position_count
            )
        session.commit()
...
if __name__ == "__main__":
    ...
    logger.info(
        f"FIN - Création / mise à jour des excursions et des segments en {duration:.2f}s"
    )
