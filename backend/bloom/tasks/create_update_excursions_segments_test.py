...
def run():
    ...
    with db.session() as session:
        ...
        while timestamp < process_start:
            logger.info(f"Lecture des nouvelles positions entre le {timestamp} et {timestamp + timedelta(days=1)}")
            ...
            for segment, zones in result.items():
                ...
                vessel_attributes = segment_repository.get_vessel_attribute_by_segment_created_updated_after(
                    session, segment.id, point_in_time
                ...
                for zone in zones:
                    ...
                    new_metricss.append(new_metric)
            ...
            TaskExecutionRepository.set_point_in_time(
                session, "create_update_excursions_segments", timestamp
            )
            session.commit()
            if point_in_time:
            ...
            if now != None:
                TaskExecutionRepository.set_position_count(
                    session, "create_update_excursions_segments", timestamp, position_count
                )
            session.commit()
