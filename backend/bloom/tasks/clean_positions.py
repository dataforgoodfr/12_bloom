...
def run(batch_time):
    ...
    pd.set_option('display.max_columns', None) # temporaire
    engine = create_engine("postgresql://bloom:bloom@localhost:5432/bloom") # temporaire
    ...
    for mmsi in batch['vessel_mmsi'].unique():
        ...
        if (last_segment.shape[0] > 0):
            vessel_last_segment = last_segment.loc[last_segment['mmsi'] == mmsi,['mmsi','timestamp_end','heading_at_end','speed_at_end','end_position','excursion_id','arrival_port_id']]
        ...
        if (df.shape[0]>0):
            ...
            def get_port(x):
                if ((x.average_speed < maximal_speed_to_check_if_in_port) | (x.type == 'DEFAULT_AIS')):
                    ...
                    query = 'SELECT id,ST_Distance(ST_POINT('+str(x.end_longitude)+','+str(x.end_latitude)+', 4326)::geography, geometry_point::geography) from dim_port where ST_Within(ST_POINT('+str(x.end_longitude)+','+str(x.end_latitude)+', 4326),geometry_buffer) = true AND ST_Distance(ST_POINT('+str(x.end_longitude)+','+str(x.end_latitude)+', 4326)::geography, geometry_point::geography) <'+str(threshold_distance_to_port)
                    with db.session() as session:
            ...
            for a in df.index:
                if (df['port'].iloc[a] >= 0):
                    if (open_ongoing_excursion):
                        close_excursion(ongoing_excursion_id,df['port'].iloc[a],df['end_latitude'].iloc[a],df['end_longitude'].iloc[a],df['timestamp_end'].iloc[a])  # put the close excursion function here
                    ...
                    if (open_ongoing_excursion):
                        ...
                        if (is_new_vessel):
                            ongoing_excursion_id = add_excursion(vessel_id,df['timestamp_end'].iloc[a],Point(df['end_longitude'].iloc[a],df['end_latitude'].iloc[a])) # put the create new excursion function here
                            #ongoing_excursion_id = 1 + mmsi*a # temp excursion id generation
                            is_new_vessel = False
                        else:
                            def get_time_of_departure():
                                if (df['type'].iloc[a] == 'DEFAULT_AIS'):
                                    return df['timestamp_end'].iloc[a] - timedelta(0,3600 * df['distance'].iloc[a] / average_exit_speed)
                                else:
                                    return df['timestamp_start'].iloc[a]
                            ongoing_excursion_id = add_excursion(vessel_id,get_time_of_departure()) # put the create new excursion function here
