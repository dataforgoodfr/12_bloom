/*
    Script : Passage en partitionnement **mensuel**
    Contexte : Remplace le partitionnement trimestriel de `staging.stg_vessel_positions` par un partitionnement **mensuel**.
...
create index if not exists pos_last_idx
    on staging.stg_vessel_positions
       (vessel_id, position_timestamp DESC, position_id);
